...
- **B3 — Stripe billing. IN CORSO (audit fatto).** Stripe NON e greenfield: libreria `stripe ^22.2.0`, client, webhook firmato `/api/stripe/webhook` e checkout (`action checkout`, riga 1822) GIA ESISTONO ma cablati sul modello VECCHIO (per-sito, prezzi 149€ una-tantum + 49€/anno, `grantEntitlement(siteId)`). Delta B3: (1) nuovi prezzi 19€/3-siti + tier su Stripe; (2) checkout per-account (passa email + metadata.maxPublished, non siteId); (3) webhook chiama `setAccountMaxPublished(email,n)` invece di `grantEntitlement(siteId)`, e su subscription.deleted rimette maxPublished=0. DECISIONE APERTA: modello prezzi = un prodotto per tier (A, 19/3-39/10-79/30 distinti) confermato come unico coerente col pricing non-lineare; da decidere se lanciare con solo 19€/3 o tutti i tier subito.
