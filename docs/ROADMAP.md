...
- **B2 — Gate account/siti pubblicati.** Forma reale (deciso 2026-06-24): NON metering modifiche (illimitate per ora), ma piano per-account. Introduce `accountPlan` (n. siti pubblicati permessi, default 3) + enforcement al publish (blocca il sito oltre il limite del piano) + override `entitled` esistente (eccezione manuale operatore) + riconciliazione trial (oggi per-sito in `siteSession.ts`, va portato a livello account). `entitled` resta come override per-sito, non viene sostituito. Cap modifiche introdotto solo se i dati mostrano abuso. Spec dettagliata nella Issue/branch `block/b2`.
